...
export function removeClass<T extends Element>(el: T, className: string): T {
  ...
  if (classList) {
    ...
    el.setAttribute(
      'class',
      (el.getAttribute('class') || '').replace(new RegExp('(\\s|^)' + className + '(\\s|$)', 'g'), '$2')
    );
  }
  return el;
}
...
export function safeSetStyle(el: HTMLElement | SVGElement, property: string, value: string) {
  if (property in el.style) {
    ...
    console.warn(
      new Error(
        `Trying to set non-existing style ` + `${property} = ${value} on a <${el.tagName.toLowerCase()}>.`
      )
    );
  }
}
...
export function safeSetAttribute(el: HTMLElement | SVGElement, name: string, value: string) {
  if (!(name in el)) {
    ...
    console.warn(
      new Error(
        `Trying to set non-existing attribute ` + `${name} = ${value} on a <${el.tagName.toLowerCase()}>.`
      )
    );
  }
...
export function makeElement(tagName: string, css: StringOrNumberMap = {}) {
