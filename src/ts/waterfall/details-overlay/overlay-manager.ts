...
class OverlayManager {
  private static showFullName = (el: Element) => {
    el.getElementsByClassName('row-fixed')?.item(0)?.dispatchEvent(new MouseEvent('mouseenter'));
  };
  /**
  ...
  public closeOverlay(index: number, detailsHeight: number, rowItems: SVGAElement[]) {
    ...
    this.openOverlays.splice(
      this.openOverlays.reduce((prev: number, curr, i) => {
    ...
    const addNewOverlay = (overlayHolder: SVGGElement, overlay: OpenOverlay) => {
      ...
      if (previewImg && !previewImg.src) {
        previewImg.setAttribute(
          'src',
          (previewImg.attributes.getNamedItem('data-src') || { value: '' }).value
        );
      }
      (infoOverlay.querySelector('a') as HTMLAnchorElement).addEventListener(
        'keydown',
        OverlayManager.firstElKeypress
      );
      (getLastItemOfNodeList(infoOverlay.querySelectorAll('button')) as HTMLButtonElement).addEventListener(
        'keydown',
        OverlayManager.lastElKeypress
      );
    ...
    const updateRow = (rowItem: SVGAElement, index: number) => {
      ...
      if (overlay === undefined) {
        if (overlayEl && nextRowItem !== null) {
          ...
          (nextRowItem.querySelector('a') as HTMLAnchorElement).removeEventListener(
            'keydown',
            OverlayManager.firstElKeypress
          );
          (getLastItemOfNodeList(
            nextRowItem.querySelectorAll('button')
          ) as HTMLButtonElement).removeEventListener('keydown', OverlayManager.lastElKeypress);
